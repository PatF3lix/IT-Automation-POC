...
function displayOnboarding(data) {
    ...
    document.getElementById("result-employee-id").textContent = onboarding.employee_id;
    document.getElementById("result-onboarding-status").textContent = onboarding.status;
    document.getElementById("result-created-at").textContent = onboarding.created_at || "-";
    document.getElementById("result-completed-at").textContent = onboarding.completed_at || "not completed";
...
if (findOnboardingForm) {
    ...
    findOnboardingForm.addEventListener("submit", async function(event) {
        ...
        const onboardingId = document.getElementById("search-onboarding-id").value;

        try {
            const response = await fetch(  `/onboardings/${onboardingId}`);
            const data = await response.json();
...
async function completeTask(taskId){
    try {
        ...
        const onboardingId = document.getElementById("result-onboarding-id").textContent;
        
        const refreshResponse = await fetch(`/onboardings/${onboardingId}`);
...
if(completeOnboardingForm){
    ...
    completeOnboardingForm.addEventListener("submit", async function(event) {
        ...
        const onboardingId = document.getElementById("complete-onboarding-id").value;

        try {
            const response = await fetch( `/onboardings/${onboardingId}/complete`,
                {method: "POST"}
            );
